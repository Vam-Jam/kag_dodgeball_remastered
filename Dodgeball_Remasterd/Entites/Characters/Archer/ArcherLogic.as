...
{
	CBlob@ nothing = server_CreateBlobNoInit("nothing");
	if (arrowType == BallType::balld)
	{
		CBlob@ arrowd = server_CreateBlobNoInit("balld");
		arrowd.set_u8("arrow type", arrowType);
		arrowd.SetDamageOwnerPlayer(this.getPlayer());
		arrowd.Init();

		arrowd.IgnoreCollisionWhileOverlapped(this);
		arrowd.server_setTeamNum(this.getTeamNum());
		arrowd.setPosition(arrowPos);
		arrowd.setVelocity(arrowVel);
		if(arrowVel.x > 0)
		{
			arrowd.setAngularVelocity(10.0f + XORRandom(20.0f));
		}
		else if(arrowVel.x < 0)
		{
			arrowd.setAngularVelocity(-10.0f + XORRandom(-20.0f));
		}
		else
	...
	if (arrowType == BallType::ballg)
	{
		CBlob@ arrowg = server_CreateBlobNoInit("ballg");
		arrowg.set_u8("arrow type", arrowType);
		arrowg.SetDamageOwnerPlayer(this.getPlayer());
		arrowg.Init();
		arrowg.setAngularVelocity(1.0f + XORRandom(20.0f));

		arrowg.IgnoreCollisionWhileOverlapped(this);
		arrowg.server_setTeamNum(this.getTeamNum());
		arrowg.setPosition(arrowPos);
		arrowg.setVelocity(arrowVel);
		if(arrowVel.x > 0)
		{
			arrowg.setAngularVelocity(10.0f + XORRandom(20.0f));
		}
		else if(arrowVel.x < 0)
		{
			arrowg.setAngularVelocity(-10.0f + XORRandom(-20.0f));
	...
	if (arrowType == BallType::balle)
	{
		CBlob@ arrowe = server_CreateBlobNoInit("balle");
		arrowe.set_u8("arrow type", arrowType);
		arrowe.SetDamageOwnerPlayer(this.getPlayer());
		arrowe.Init();
		arrowe.setAngularVelocity(1.0f + XORRandom(20.0f));

		arrowe.IgnoreCollisionWhileOverlapped(this);
		arrowe.server_setTeamNum(this.getTeamNum());
		arrowe.setPosition(arrowPos);
		arrowe.setVelocity(arrowVel);
		if(arrowVel.x > 0)
		{
			arrowe.setAngularVelocity(10.0f + XORRandom(20.0f));
		}
		else if(arrowVel.x < 0)
		{
			arrowe.setAngularVelocity(-10.0f + XORRandom(-20.0f));
	...
	if (arrowType == BallType::ballb)
	{
		CBlob@ arrowb = server_CreateBlobNoInit("ballb");
		arrowb.set_u8("arrow type", arrowType);
		arrowb.SetDamageOwnerPlayer(this.getPlayer());
		arrowb.Init();
		arrowb.setAngularVelocity(1.0f + XORRandom(20.0f));

		arrowb.IgnoreCollisionWhileOverlapped(this);
		arrowb.server_setTeamNum(this.getTeamNum());
		arrowb.setPosition(arrowPos);
		arrowb.setVelocity(arrowVel);
		if(arrowVel.x > 0)
		{
			arrowb.setAngularVelocity(10.0f + XORRandom(20.0f));
		}
		else if(arrowVel.x < 0)
		{
			arrowb.setAngularVelocity(-10.0f + XORRandom(-20.0f));
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	...
	CGridMenu@ menu = CreateGridMenu(pos, this, Vec2f(ballTypeNames.length, 2), "Current arrow");
	...
	if (menu !is null)
	{
		...
		for (uint i = 0; i < ballTypeNames.length; i++)
		{
			string matname = ballTypeNames[i];
			CGridButton @button = menu.AddButton(arrowIcons[i], arrowNames[i], this.getCommandID("pick " + matname));

			if (button !is null)
			{
				bool enabled = this.getBlobCount(ballTypeNames[i]) > 0;
