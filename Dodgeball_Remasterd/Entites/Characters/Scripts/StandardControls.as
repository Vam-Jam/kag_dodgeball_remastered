...
void AdjustCamera(CBlob@ this)
{
	CCamera@ camera = getCamera();
	float targetDistance = camera.targetDistance;
	if(targetDistance > 0.75f)
	{
		camera.targetDistance -= 0.0015f;
	}
	else
	{
		camera.targetDistance = 0.75f;
	}
}
...
void ManageCamera(CBlob@ this)
{
	...
	u16 charge = this.get_u16("charge");
	//float cameraScale = this.get_u32("mScale");
	if(charge > 0)
	{
		if(charge < 45)
		{
			camera.targetDistance += 0.0005f;
		}
	}
	else
	{
